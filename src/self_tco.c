...
simple_type_p(int src){
    return (src == TIDENT) || (src == TSTRING) || (src == TINT) || (src == TFLOAT) || (src == TARRAYLITERAL) || (src == TCHAR) || (src == TBOOL) || (src == THEX) || (src == TOCT) || (src == TBIN);
}
AST * 
...
rewrite_tco(AST * src){
    ...
    body->children[1] = copy_body(src->children[1], src->children[0], true);
    return ret;
}
